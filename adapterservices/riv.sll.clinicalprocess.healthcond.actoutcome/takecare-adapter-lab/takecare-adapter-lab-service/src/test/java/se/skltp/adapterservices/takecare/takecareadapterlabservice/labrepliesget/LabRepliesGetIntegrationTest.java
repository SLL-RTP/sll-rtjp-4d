...
public class LabRepliesGetIntegrationTest extends AbstractTestCase {
    ...
    private static final Logger log = LoggerFactory.getLogger(LabRepliesGetIntegrationTest.class);
    ...
    protected String getConfigResources() {
        return "soitoolkit-mule-jms-connector-activemq-embedded.xml," + "takecare-adapter-lab-service-common.xml,"
                + "LabRepliesGet-service.xml," + "teststub-services/LabRepliesGet-teststub-service.xml";
    }
    ...
    public void test_ok() throws Fault {
        ...
        LabRepliesGetTestConsumer consumer = new LabRepliesGetTestConsumer(DEFAULT_SERVICE_ADDRESS);

        GetClinicalChemistryLabOrderOutcomeResponseType response = consumer.callService(patientId, null, null);
        List<LabResultsType> labResults = response.getLabResults();
        assertEquals(2, labResults.size());
        assertEquals(2, labResults.get(0).getAnalyses().getAnalysis().size());
    ...
    public void test_fault_invalidInput() throws Exception {
        try {
            ...
            LabRepliesGetTestConsumer consumer = new LabRepliesGetTestConsumer(DEFAULT_SERVICE_ADDRESS);
            GetClinicalChemistryLabOrderOutcomeResponseType response = consumer.callService(TEST_ID_FAULT_INVALID_ID,
                    null, null);
    ...
    public void test_fault_timeout() throws Fault {
        try {
            LabRepliesGetTestConsumer consumer = new LabRepliesGetTestConsumer(DEFAULT_SERVICE_ADDRESS);
            GetClinicalChemistryLabOrderOutcomeResponseType response = consumer.callService(TEST_ID_FAULT_TIMEOUT,
                    null, null);
            fail("expected fault, but got a response of type: "
                    + ((response == null) ? "NULL" : response.getClass().getName()));
        } catch (SOAPFaultException e) {

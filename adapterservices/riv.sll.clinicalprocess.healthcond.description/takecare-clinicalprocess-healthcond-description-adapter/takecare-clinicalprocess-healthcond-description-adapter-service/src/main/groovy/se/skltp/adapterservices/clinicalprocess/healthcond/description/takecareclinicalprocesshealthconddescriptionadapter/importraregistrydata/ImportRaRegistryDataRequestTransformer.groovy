  ...
  String transformXml(String xml, String logicalAddress) {
    ...
    try    {
      ...
      def envelope = builder.bind {
        ...
        mkp.declareNamespace(core:'urn:riv:sll:clinicalprocess:healthcond:description:1')
        mkp.declareNamespace(itr:'urn:riv:itintegration:registry:1')
        mkp.declareNamespace(tns:'urn:riv:sll:clinicalprocess:healthcond:description:RegisterRaDSDataResponder:1')
              ...
              if (1 == inRegistryData.Patient.size())
                'tns:patient' {
                  ...
                  if (1 == patient.ID.size()) {
                    'core:personId' {
                        'core:id' patient.ID.text()
                        'core:type' '1.2.752.129.2.1.3.1'
                    }
                  }
              ...
              if (1 == inRegistryData.Data.size())
                'tns:registryData' {
                      ...
                      data.Diagnosis.each{
                        ...
                        'tns:diagnosis' {
                          ...
                          if (1 == d.OnsetDate.size())
                            'tns:onsetDate'         parseDate(d.OnsetDate.text()).format('yyyyMMdd') 
                        }
                      }
                    }
                  ...
                  if (1 == data.VisitDate.size())
                    'tns:visitDate'               parseDate(data.VisitDate.text()).format('yyyyMMdd')
                  ...
                  if (1 == data.XrayDate.size())
                    'tns:xrayDate'                parseDate(data.XrayDate.text()).format('yyyyMMdd')
                      ...
                      data.Drug.each{
                        ...
                        'tns:drug' {
                          ...
                          if (1 == drug.StartDate.size())
                            'tns:startDate'        parseDate(drug.StartDate.text()).format('yyyyMMdd')
                          if (1 == drug.EndDate.size())
                            'tns:endDate'          parseDate(drug.EndDate.text()).format('yyyyMMdd')
